...
public class SpawnZombie : MonoBehaviour
{
    ...
    public float TimeToSpawnZombie, SpawnCountdown; // creating a countdown and resetting it so zombie star spawns when timer reaches 0
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   
    // Start is called once before the first execution of Update after the MonoBehaviour is created
    void Start()
    {
        SpawnCountdown = TimeToSpawnZombie; // when countdown reaches 0, reset TimeToSpawnZombie to variable in inspector.
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    void Update()
    {
        SpawnZombieStar(); // call the zombie spawn function

    }
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   
    public void SpawnZombieStar() // create the spawn zombie function
    {
        ...
        {
            SpawnCountdown = TimeToSpawnZombie; // reset the timer when it reaches 0
            Instantiate(objectToSpawn, ZombieSpawnPoint.position, ZombieSpawnPoint.rotation); // spawn zombiestar at ZombieSpawnPoint
        }
    }
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------   
}
